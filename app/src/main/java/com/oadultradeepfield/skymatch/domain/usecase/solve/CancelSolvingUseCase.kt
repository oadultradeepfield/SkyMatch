 ...
 */
class CancelSolvingUseCase @Inject constructor(private val repo: ISolveRepository) {
  /**
   * Cancels the solving of images for the given job IDs.
   *
   * @param jobIds The list of job IDs to cancel.
   * @return A list of results, where each result is either a success or a failure with an
   *   exception.
   */
  suspend operator fun invoke(
      jobIds: List<String>,
  ): List<Result<Unit>> = coroutineScope {
    jobIds.map { jobId -> async { runCatching { repo.cancelSolving(jobId) } } }.awaitAll()
  }
}
